...
static void waitFor(int hours, int minutes, int seconds, int tenths)
{
    ...
    for (i = 0; i < ticks; i++) {
        ...
        EWatchAlarm_Dispatch(&ewatch_alarm, AL_CLOCK_TICK_SIG);
    }
}
...
static void setUp(void)
{
    ...
    EWatchAlarm_Init(&ewatch_alarm, &external);
}
...
TEST_CASE("Set alarm to time", "[alarm]")
{
    ...
    setAlarmTo(&ewatch_alarm, 11, 0);

    TEST_ASSERT_EQUAL(11, EWatchTimeset_GetHours(&ewatch_alarm.expirationTime));
    TEST_ASSERT_EQUAL(0, EWatchAlarm_GetMinutes(&ewatch_alarm));
    TEST_ASSERT_EQUAL(ALARM_ON, EWatchAlarm_GetAlarmState(&ewatch_alarm));
}
...
TEST_CASE("Switch AlarmState to 'on' when hour and minutes are set, and an additional confirmation signal is received", "[alarm]")
{
    ...
    EWatchAlarm_Dispatch(&ewatch_alarm, AL_ALARM_SET_SIG); // set hours
    EWatchAlarm_Dispatch(&ewatch_alarm, AL_ALARM_SET_SIG); // set minutes

    TEST_ASSERT_EQUAL(ALARM_OFF, EWatchAlarm_GetAlarmState(&ewatch_alarm)); // alarm is still 'off'

    EWatchAlarm_Dispatch(&ewatch_alarm, AL_ALARM_SET_SIG); // confirm alarm
    TEST_ASSERT_EQUAL(ALARM_ON, EWatchAlarm_GetAlarmState(&ewatch_alarm)); // alarm is 'on'
}
...
TEST_CASE("Set alarm, wait for ticks, and expire", "[alarm]")
{
    ...
    setAlarmTo(&ewatch_alarm, 12, 0);

    TEST_ASSERT_EQUAL_MESSAGE(ALARM_ON, EWatchAlarm_GetAlarmState(&ewatch_alarm), "Expected ALARM_OFF");

    waitFor(12, 0, 0, 0);

    TEST_ASSERT_EQUAL_MESSAGE(ALARM_EXPIRED, EWatchAlarm_GetAlarmState(&ewatch_alarm), "Expected ALARM_EXPIRED");
}
...
TEST_CASE("Alarm will expire if external time is changed", "[alarm]")
{
    ...
    unsigned int twelve = EWatchTimeset_GetCount(&ewatch_alarm.expirationTime);
    ...
    enum AlarmState alarmState = EWatchAlarm_GetAlarmState(&ewatch_alarm);
...
TEST_CASE("Once alarm expires, an AlarmSet signal turns off the alarm", "[alarm]")
{
    ...
    setAlarmTo(&ewatch_alarm, 5, 30);

    waitFor(0, 1, 0, 0);

    TEST_ASSERT_EQUAL_MESSAGE(ALARM_EXPIRED, EWatchAlarm_GetAlarmState(&ewatch_alarm), "Expected ALARM_EXPIRED");

    EWatchAlarm_Dispatch(&ewatch_alarm, AL_ALARM_SET_SIG); // Shutdown

    TEST_ASSERT_EQUAL_MESSAGE(ALARM_OFF, EWatchAlarm_GetAlarmState(&ewatch_alarm), "Expected ALARM_OFF");
}
